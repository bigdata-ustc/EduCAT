...
class IRT(nn.Module):
    def __init__(self, num_students, num_questions, num_dim,config):
...
class env:
    ...
    def load_CDM(self,name,data,pth_path,config):
        if name == 'IRT':
            model = IRT(data.num_students, data.num_questions, self.config['num_dim'],config)
            model.load_state_dict(torch.load(pth_path), strict=False)
        #self.policy.policy.load_state_dict(torch.load('policy.pt'),strict=False)
...
class NCATModel():
    ...
    def __init__(self, NCATdata,concept_map,config,test_length):
        ...
        self.fa = NCAT(n_question=NCATdata.num_questions+1).to(device)
    ...
    def ncat_policy(self,sid,THRESHOLD,used_actions):
        ...
        while not done:
            ...
            data["uid"] = torch.tensor(data["uid"], device=device)
